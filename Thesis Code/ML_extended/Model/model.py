...
# Default harmonic order used by dataset feature generation.
HARMONICS = 3
...
class SolarFluxDataset(Dataset):
    def __init__(self, filename, seq_length, forecast_horizon,
                 ...
                 kp_ap_file='dst_index_daily.csv', sunspot_file='SN_d_tot_V2.0.csv'):
        ...
        all_columns = columns + ['Kp','Total','Dst']  # Kp, Total (sunspot), and Dst are exogenous
        ...
        # Initialize normalizatio
        self.mean   = data.mean(axis=0, keepdims=True, dtype=np.float32)
        self.std    = data.std(axis=0, keepdims=True, dtype=np.float32)
        self.series = ((data - self.mean) / self.std).astype(np.float32)
        ...
        self.harmonics = np.concatenate([self.doy_sin, self.doy_cos, self.cyc_sin, self.cyc_cos], axis=1).astype(np.float32)
        ...
        self._rebuild_feature_tensors()

    def _rebuild_feature_tensors(self):
        # Keep normalized series in NumPy and prebuild Torch tensors for fast slicing in __getitem__.
        self.series = self.series.astype(np.float32, copy=False)
        self.series_torch = torch.from_numpy(self.series)
        self.harmonics_torch = torch.from_numpy(self.harmonics) * self._cyc_scale
        self.augmented_torch = torch.cat([self.series_torch, self.harmonics_torch], dim=1)

    def set_normalization_from_indices(self, train_indices):
        train_data = self.raw[train_indices]
        self.mean = train_data.mean(axis=0, keepdims=True, dtype=np.float32)
        self.std = train_data.std(axis=0, keepdims=True, dtype=np.float32)
        # Recompute normalized series with training-only statistics
        self.series = ((self.raw - self.mean) / self.std).astype(np.float32)
        self._cyc_scale = float(self.std.mean())
        self._rebuild_feature_tensors()
    ...
    def __getitem__(self, idx):
        x_tensor = self.augmented_torch[idx : idx + self.seq_length].transpose(0, 1)
        ...
        y_tensor = self.series_torch[y_start : y_end, :self.n_target_vars]
...
class SolarFluxDataModule(pl.LightningDataModule):
    def __init__(self, filename, seq_length, pred_length,
        ...
        self.target_columns = target_columns or ['Rotation','Obs','Adj','URSI-D','ap']
...
if __name__ == '__main__':
    ...
    filename       = os.path.join(data_root, 'SW-All.csv')
    kp_ap_file     = os.path.join(data_root, 'dst_index_daily.csv')
    sunspot_file   = os.path.join(data_root, 'SN_d_tot_V2.0.csv')
    columns        = ['Obs','Adj','URSI-D','ap']  # Added 'ap' to target variables
    all_columns    = columns + ['Kp','Total', 'Dst', 'Rotation']  # Kp, Total, Dst, and Rotation are exogenous
    ...
    out_vars    = dm.full_ds.n_target_vars
    ...
    for fold, (train_idx, val_idx) in enumerate(tscv.split(dm.full_ds)):
        ...
        cb_early = EarlyStopping('val_loss', patience=10, mode='min')
    ...
    # Generate visualizations and save predictions (including conditional Ap forecast)
    generate_visualizations_and_predictions(final_model, dm, loss_history, cv_losses, cv_maes, 
                                            forecast_years, pred_len, colors, HARMONICS,
                                            ap_regression_model=regression_model, ap_ar_coeffs=ar_coeffs,
                                            ap_sigma=sigma, ap_eps_init=eps_init, simulate_ap_fn=simulate_ap_forecast,
                                            last_train_idx=last_train_idx, df_hist=df_hist)