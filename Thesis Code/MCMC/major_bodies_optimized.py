...
def extract_state_vector(vec, au, day):
    # Optimization: Access columns by name once, create array directly
    # 'vec' is an astropy table, accessing columns is fast
    pos = np.array([vec['x'][0], vec['y'][0], vec['z'][0]], dtype=np.float64) * au
    vel = np.array([vec['vx'][0], vec['vy'][0], vec['vz'][0]], dtype=np.float64) * (au / day)
...
def load_mb(frame, start_date):
    ...
    # Optimization: Use ThreadPoolExecutor to fetch data in parallel.
    # This significantly reduces runtime by overlapping network latency.
