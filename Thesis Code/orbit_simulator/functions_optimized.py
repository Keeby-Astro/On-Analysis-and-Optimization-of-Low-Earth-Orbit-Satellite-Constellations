...
def xyz2orb(mu, r_vec, v_vec):
    ...
    xyz2orb_into(mu, r_vec, v_vec, out)
    return out


@njit(cache=True, fastmath=True, nogil=True, inline="always")
def xyz2orb_into(mu, r_vec, v_vec, out):
    ...
    omega = normalize_angle(np.arctan2(ny, nx))
    ...
    Me = normalize_angle(Ea - e * np.sin(Ea))
...
def orb2xyz(mu, oe):
    out = np.empty(6, dtype=np.float64)
    orb2xyz_into(mu, oe, out)
    return out


@njit(cache=True, fastmath=True, nogil=True, inline="always")
def orb2xyz_into(mu, oe, out):
    # Unpack elements
    ...
    out[0] = R11 * r_p_x + R12 * r_p_y
    out[1] = R21 * r_p_x + R22 * r_p_y
    out[2] = R31 * r_p_x + R32 * r_p_y
    out[3] = R11 * v_p_x + R12 * v_p_y
    out[4] = R21 * v_p_x + R22 * v_p_y
    out[5] = R31 * v_p_x + R32 * v_p_y
