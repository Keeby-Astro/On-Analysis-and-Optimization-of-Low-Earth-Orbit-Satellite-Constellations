...
if __name__ == '__main__':
    ...
    columns        = ['Obs', 'Adj', 'URSI-D', 'ap']
    batch_size     = 128
    checkpoint_path = 'solar_flux_model.pth'

    if not os.path.exists(checkpoint_path):
        raise FileNotFoundError(f"Checkpoint not found: {checkpoint_path}")

    # Load checkpoint metadata + model weights
    checkpoint = torch.load(checkpoint_path, map_location='cpu')
    cnn_params = checkpoint['cnn_params']
    ts_params  = checkpoint['ts_params']
    in_vars    = checkpoint['in_vars']
    out_vars   = checkpoint['out_vars']
    seq_len    = checkpoint['seq_len']
    pred_len   = ts_params['pred_len']
    lr         = checkpoint.get('lr', 1e-4)

    # Build datamodule for inference
    ...
    # Run one validation batch as a sanity-check inference
    ...
    print(f"Inference complete on one validation batch.")
    print(f"x shape: {tuple(x.shape)}")
    print(f"y shape: {tuple(y.shape)}")
    print(f"y_hat shape: {tuple(y_hat.shape)}")
    print(f"Batch MAE: {mae:.6f}")